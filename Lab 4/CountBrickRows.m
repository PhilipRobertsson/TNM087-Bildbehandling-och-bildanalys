...
%Blue channel gives the best contrasts

%% Lowpass filter your image
krn = ones(5)/(5^2); % 5x5 box kernel
bfilt=filter2(krn,bgray,'same'); % The filtered version of the grayscale image

%% Threshold your image
% to separate the mortar joints from the bricks
T = graythresh(bfilt);

b_thresh= T<bfilt; % The thresholded image
%% Use Hough transform to find the angle corresponding to the principal lines
[H, t, ~] = hough(b_thresh,'Rhoresolution', 5, 'Theta', -90:0.5:89.5);
H1 = ((H-min(H(:)))/max(H(:))-min(H(:)));
%Hough transform according to lecture 8


[~,teta]=find(H1==max(H1(:))); % The angle corresponding to the principal lines (rows of mortar joints)
...
if(t(teta) < 0)
    trot = 90 + t(teta); % Angle of rotation.
else
    trot =t(teta) - 90;
end
...
IMG= imrotate(in,trot,"nearest","crop"); % The rotated image to the horizontal level (the first output argument of this funciton)
...
sum_thresh=0.6<sum_row; % The thresholded version of sum_row
% Display this graph by plot, but remove or comment before submitting the
% file
...
deri_krn = [-1;1]; % Approximation of the first derivative
sum_deri = filter2(deri_krn,sum_thresh,"same"); % The first derivative applied to the summation
nofr=numel(find(sum_deri==max(sum_deri(:)))) -1; % The number of rows of the bricks (the seond output argument of this function)
% 1 is subtracted from the total amount of brick rows because we are using
% an approximation.
